...
Sphere spheres[] = {
    ...
    Sphere(16.5, Vec(50, 16.5, 100), Vec(), Vec(1, 1, 1) * .999, REFR),        //Glas
...
Vec radiance(const Ray &r, int depth, unsigned short *Xi)
{
    ...
    { // Ideal DIFFUSE reflection
        ...
        Vec u = ((fabs(w.x) > .1 ? Vec(0, 1) : Vec(1)) % w).norm(), v = w % u;
        // TODO: what is this?
...
int main(int argc, char *argv[])
{
    ...
    // cx: x-direction "unit" vector on the canvas, cy: y-direction "unit" vector on the canvas
    
    // 0.5135 used to adjust the canvas's actual size
    Vec cx = Vec(w * .5135 / h), cy = (cx % cam.d).norm() * .5135;
    // Vec cx = Vec(w * .3 / h), cy = (cx % cam.d).norm() * .3;
    Vec r, *c = new Vec[w * h];
#pragma omp parallel for schedule(dynamic, 1) private(r) // OpenMP
    ...
    { // Loop over image rows
        ...
        for (unsigned short x = 0, Xi[3] = {0, 0, y * y * y}; x < w; x++) // Loop cols
            for (int sy = 0, i = (h - y - 1) * w + x; sy < 2; sy++)       // 2x2 subpixel rows, i = index of pixels unrolled
                ...
                { // 2x2 subpixel cols
                    for (int s = 0; s < samps; s++)
                    {
                        ...
                        // move cam.o to appropriate location
                        r = r + radiance(Ray(cam.o + d * 140, d.norm()), 0, Xi) * (1. / samps);
                        // r = r + radiance(Ray(cam.o + d * 200, d.norm()), 0, Xi) * (1. / samps);
